...
func registerMemoryRoutes(api *gin.RouterGroup) {
	...
	// 截图高亮：返回每个 fact 对应的匹配消息索引
	api.GET("/memory/:id/matched", func(c *gin.Context) {
		id, err := strconv.Atoi(c.Param("id"))
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "id 非法"})
			return
		...
		// 从 vec_messages 加载该段消息的 embedding
		limit := sourceTo - sourceFrom + 1
		if limit <= 0 {
			c.JSON(http.StatusOK, gin.H{"matched_indices": []int{}})
			return
		}
		embRows, embErr := db.Query(
			`SELECT embedding FROM vec_messages WHERE contact_key = ? ORDER BY seq LIMIT ? OFFSET ?`,
			contactKey, limit, sourceFrom)
		if embErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": embErr.Error()})
			return
		}
		var msgVecs [][]float32
		for embRows.Next() {
			var blob []byte
			embRows.Scan(&blob)
			msgVecs = append(msgVecs, decodeVec(blob))
		}
		embRows.Close()

		if len(msgVecs) == 0 || len(relatedFacts) == 0 {
			c.JSON(http.StatusOK, gin.H{"matched_indices": []int{}})
			return
		}

		// 为每个 fact 获取 embedding（提取核心事实文本，去掉时间前缀）
		prefs := loadPreferences()
		embConfigs := embeddingConfigs(prefs)
		factTexts := make([]string, len(relatedFacts))
		for i, f := range relatedFacts {
			// 去掉 [time range] 前缀
			if len(f) > 0 && f[0] == '[' {
				idx := strings.Index(f, "] ")
				if idx > 0 {
					factTexts[i] = strings.TrimSpace(f[idx+2:])
					continue
				}
			}
			factTexts[i] = strings.TrimSpace(f)
		}

		factVecs, embErr := GetEmbeddingsBatchWithFallback(factTexts, embConfigs)
		if embErr != nil || len(factVecs) != len(factTexts) {
			c.JSON(http.StatusOK, gin.H{"matched_indices": []int{}})
			return
		}

		// 对每条消息，检查是否与任一 fact 相似
		const similarityThreshold = 0.3
		matchedIndices := []int{}
		for i, msgVec := range msgVecs {
			if len(msgVec) == 0 {
				continue
			}
			for _, factVec := range factVecs {
				if len(factVec) != len(msgVec) {
					continue
				}
				sim := cosineSimilarity(factVec, msgVec)
		...
		c.JSON(http.StatusOK, gin.H{"matched_indices": matchedIndices})
	})
}
